...
class ShadowManifest(BaseModel):
    ...
    compile_targets: dict[str, bool] = Field(default_factory=dict)


# ---------------------------------------------------------------------------
# Stack compatibility checker (Kotlin / AGP / Compose vs target dependency)
# ---------------------------------------------------------------------------
...
class StackCompatibilityReport(BaseModel):
...
class FileImpact(BaseModel):
    ...
    def impact_level(self) -> int:
        """0 = transitive (or expect/actual / unknown), 2 = direct, 1 = transitive."""
        if self.relation == ImpactRelation.DIRECT:
            return 2
        return 1
...
class UTGNode(BaseModel):
    ...
    state_str: str = ""
    screen_name: str = ""
    structure_id: str = ""  # stable hash of the view tree — same UI shell → same id
    title_hint: str = ""    # best-effort human label extracted from visible text
...
class UIRegressions(BaseModel):
    ...
    activity_coverage_before: dict[str, int] = Field(default_factory=dict)
    activity_coverage_after: dict[str, int] = Field(default_factory=dict)
    nodes_before: int = 0
    nodes_after: int = 0
    structures_before: int = 0
    structures_after: int = 0
...
class ConsolidatedResult(BaseModel):
    ...
    stack_compatibility: StackCompatibilityReport = Field(default_factory=StackCompatibilityReport)
...
class CCAttribute(BaseModel):
    ...
    impact_level: int = 0  # 0=no impact, 1=transitive, 2=direct
