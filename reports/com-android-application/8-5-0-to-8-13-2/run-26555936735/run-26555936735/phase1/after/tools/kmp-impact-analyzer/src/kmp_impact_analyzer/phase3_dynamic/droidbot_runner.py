...
def _has_utg_output(output_dir: Path) -> bool:
    return (output_dir / "utg.js").is_file() or (output_dir / "utg.json").is_file()
...
def run_dynamic_analysis(config: AnalysisConfig) -> UIRegressions:
    ...
    if pre_before and pre_before.exists() and pre_after and pre_after.exists():
        if not _has_utg_output(pre_before) or not _has_utg_output(pre_after):
            reason = "Pre-generated DroidBot output is missing utg.js/utg.json"
    ...
    def _unique_screen_names(utg) -> list[str]:
        # One entry per unique view structure — collapses repeated visits but
        # keeps separate Compose surfaces that share an Activity.
        seen: set[str] = set()
        names: list[str] = []
        for node in utg.nodes:
            ...
            if key in seen:
                continue
            seen.add(key)
            names.append(node.screen_name or node.activity or "screen")
        return names

    def _activity_coverage(utg) -> dict[str, int]:
        coverage: dict[str, int] = {}
        for node in utg.nodes:
            act = node.activity or "unknown"
            coverage[act] = coverage.get(act, 0) + 1
        return coverage

    before_structures = {n.structure_id or n.state_id for n in before_utg.nodes if n.structure_id or n.state_id}
    after_structures = {n.structure_id or n.state_id for n in after_utg.nodes if n.structure_id or n.state_id}

    result = UIRegressions(
        status=DynamicStatus.COMPLETED,
        before_screens=_unique_screen_names(before_utg),
        after_screens=_unique_screen_names(after_utg),
        diffs=diffs,
        activity_coverage_before=_activity_coverage(before_utg),
        activity_coverage_after=_activity_coverage(after_utg),
        nodes_before=len(before_utg.nodes),
        nodes_after=len(after_utg.nodes),
        structures_before=len(before_structures),
        structures_after=len(after_structures),
    )
