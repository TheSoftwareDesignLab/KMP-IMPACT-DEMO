...
def _build_project_from_paths(
    ...
    for abs_path in _collect_source_files(project_root):
        ...
        # impact_level: 0 = not impacted, 1 = transitive/expect-actual, 2 = direct.
        # CodeCharta colors a single attribute on a 2-stop gradient + neutral baseline,
        # so this discrete encoding gives us three distinguishable visual buckets
        # (white → amber → red) instead of the binary impacted/not.
        if relation == ImpactRelation.DIRECT:
            level = 2
        elif relation in (ImpactRelation.TRANSITIVE, ImpactRelation.EXPECT_ACTUAL):
            level = 1
        else:
            level = 0
