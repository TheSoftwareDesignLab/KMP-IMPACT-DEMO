...
def test_generate_report_site_creates_html_and_summary(tmp_path: Path):
    ...
    # The report HTML is now in English; we assert the major section labels
    # that the live page still uses so the test continues to validate that
    # the five-phase pipeline is rendered end-to-end.
    assert "Impact Report" in html_text
    assert "Shadow Build" in html_text
    assert "Propagation Graph" in html_text
    assert "Traceability" in html_text
