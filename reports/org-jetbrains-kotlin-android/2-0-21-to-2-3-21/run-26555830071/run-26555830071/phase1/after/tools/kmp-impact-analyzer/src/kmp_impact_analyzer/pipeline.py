...
def run_pipeline(config: AnalysisConfig) -> None:
    ...
    # Decorate any DroidBot UTG output that we have on disk (local runs and
    # CI both end up here). The CI workflow already runs this in the merge
    # job, but invoking it here means a developer running ``kmp-impact
    # analyze`` locally also sees the relation-coloured borders without an
    # extra step. ``colorize_droidbot_html`` is idempotent.
    from .reporting.utg_decorate import colorize_droidbot_html

    for sub in ("impact-utg", "after-utg", "after", "before-utg", "before"):
        target = output / "phase3" / sub
        if target.exists() and colorize_droidbot_html(target, consolidated):
            log.info(f"  Decorated phase3/{sub}/index.html with impact borders")
